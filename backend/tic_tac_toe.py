  ...
  """TTT Game State"""
    ...
    def board_status(self):
        ...
        for r in range(3):
            ...
            if three_in_row(self, self.board[r]): self.result = self.board[r][0]
            if three_in_row(self, col): self.result = self.board[0][r]
            if two_in_row(self.board[r], self.ai): self.loss.append([r, self.board[r].index("")])
            if two_in_row(self.board[r], self.user): self.win.append([r, self.board[r].index("")])
            if two_in_row(col, self.ai): self.loss.append([col.index(""), r])
            if two_in_row(col, self.user): self.win.append([col.index(""), r])
        down_r = [self.board[i][i] for i in range(3)]
        up_r = [self.board[2 - i][i] for i in range(3)]
        if three_in_row(self, down_r): self.result = down_r[0]
        if three_in_row(self, up_r): self.result = up_r[0]
        if two_in_row(self, down_r, self.ai): self.loss.append([down_r.index(""), down_r.index("")])
        if two_in_row(self, down_r, self.user): self.win.append([down_r.index(""), down_r.index("")])
        if two_in_row(self, up_r, self.ai): self.loss.append([2 - up_r.index(""), up_r.index("")])
        if two_in_row(self, up_r, self.user): self.win.append([2 - up_r.index(""), up_r.index("")])
        self.result = 'tie' if !self.result and x + o == 9
        self.ai_turn = (x > o and self.ai == 'o') or (x == o and self.ai == 'x')

    def ai_move(self):
        opposite = self.opposites_corners()
        ...
        if len(self.board[1][1]) == 0 and self.ai_turn:
            ...
            self.board[opposite[0][0]][opposite[0][1]] = self.ai
        else:
            for r in range(3):
                for c in range(3):
